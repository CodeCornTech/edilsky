...
const HERO_VIDEO_SRC = "/hero-edilsky.mp4";

function OperatorSilhouette() {
  return (
    <motion.div
      initial={{ opacity: 0, y: -36, scale: 0.82, filter: "blur(10px)" }}
    ...
    >
      ...
      <div className="relative h-28 w-20 overflow-hidden md:h-32 md:w-24 lg:h-36 lg:w-28">
        <div className="absolute inset-0 rounded-full bg-blue-500/10 blur-2xl" />
        <img
          src={OPERATOR_SRC}
          alt="operatore su fune edil sky"
          className="absolute left-1/2 top-1/2 h-[230%] max-w-none -translate-x-[52%] -translate-y-[17%] object-cover opacity-95 drop-shadow-[0_0_26px_rgba(59,130,246,0.95)]"
        />
      </div>
...
function BrandBadge() {
  return (
    <div className="rounded-full border border-white/10 bg-black/30 px-6 py-4 backdrop-blur-md md:px-10 md:py-5">
...
export default function EdilSkyLanding() {
  ...
  const operatorY = useTransform(scrollYProgress, [0, 0.18, 0.42, 0.62], ["-8vh", "18vh", "54vh", "92vh"]);
  const operatorX = useTransform(scrollYProgress, [0, 0.18, 0.36, 0.58], [0, -14, 10, 0]);
  const operatorRotate = useTransform(scrollYProgress, [0, 0.18, 0.36, 0.58], [0, -7, 5, 0]);
  const operatorScale = useTransform(scrollYProgress, [0, 0.46, 0.62], [1, 1.14, 1.85]);
  const operatorOpacity = useTransform(scrollYProgress, [0, 0.44, 0.58, 0.64], [1, 1, 0.35, 0]);

  const buildingsY = useTransform(scrollYProgress, [0, 1], ["0%", "-95%"]);
  const leftX = useTransform(scrollYProgress, [0, 0.35, 0.7], [0, -18, -34]);
  const rightX = useTransform(scrollYProgress, [0, 0.35, 0.7], [0, 18, 34]);
  const sideOpacity = useTransform(scrollYProgress, [0, 0.18, 0.55, 1], [0.18, 0.42, 0.28, 0.12]);
  const sceneScale = useTransform(scrollYProgress, [0, 0.6], [1, 1.16]);

  const badgeOpacity = useTransform(scrollYProgress, [0, 0.52, 0.66, 1], [0, 0, 1, 1]);
  const badgeScale = useTransform(scrollYProgress, [0, 0.56, 0.7], [0.72, 0.72, 1]);
  const badgeY = useTransform(scrollYProgress, [0, 0.56, 0.7], [46, 46, 0]);

  const vignetteOpacity = useTransform(scrollYProgress, [0, 0.4, 1], [0.32, 0.52, 0.8]);
  ...
  return (
    <div className="overflow-x-hidden bg-black text-white">
      ...
      <section className="py-40 text-center">
        <h2 className="text-5xl font-black uppercase">Non stai guardando una hero</h2>
        <h2 className="mt-4 text-5xl font-black uppercase text-blue-500">Stai entrando nella scena</h2>
      </section>
